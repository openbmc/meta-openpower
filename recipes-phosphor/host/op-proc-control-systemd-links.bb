...
pkg_postinst_${PN}() {
	...
	mkdir -p $D$systemd_system_unitdir/obmc-host-diagnostic-mode@.target.requires
...
pkg_prerm_${PN}() {
	...
	LINK="$D$systemd_system_unitdir/obmc-host-startmin@0.target.requires/op-continue-mpreboot@0.service"
	rm $LINK
	LINK="$D$systemd_system_unitdir/obmc-host-diagnostic-mode@.target.requires/op-enter-mpreboot@0.service"
	rm $LINK
}
